...
async def execute_finetune_generated_queries(comp_type: str, model: str):
    data = []
    path = os.path.join(FINETUNE_INPUTS, f"f{model}_results_{comp_type}.jsonl")
    print(path)
    with open(path, "r", encoding="utf-8") as f:
        for i, line in enumerate(f, start=1):
            line = line.strip()
    ...
    op_file = os.path.join(FINETUNE_OUTPUTS, f"f{model}_results_{comp_type}.jsonl")
    with open(op_file, "w", encoding="utf-8") as outfile:
