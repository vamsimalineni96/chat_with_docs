...
class PDFParser:
    ...
    def open_pdf(self):
        ...
        try:
            ...
            logger.exception("Failed to open PDF '%s': %s", self.file_path, e)
            raise PDFParseError(f"Failed to open PDF '{self.file_path}'.") from e
    ...
    def extract_text_from_pdf(self):
        ...
        try:
            ...
            logger.exception("Failed to extract text from PDF '%s': %s", self.file_path, e)
            raise PDFParseError("Failed to extract text from PDF.") from e
    ...
    def is_toc_page(text):
        ...
        dotted_lines = sum(
            ...
            if re.search(r"\.{3,}\s*\d{1,3}$", line.strip())
        )
    ...
    def parse_pdf(self):
        ...
        Raises:
            PDFParseError: if parsing fails at any step.
        """
        try:
            ...
            logger.exception("Unexpected error while parsing PDF '%s': %s", self.file_path, e)
            raise PDFParseError("Unexpected error while parsing PDF.") from e

        try:
            for p in raw_pages:
                if (
                    p["page_number"] <= self.toc_check_pages
                    and self.is_toc_page(p["text"])
                ):
                ...
                self.cleaned_pages.append(
                    {"page_number": p["page_number"], "text": cleaned_text}
                )
            ...
            if self.skipped_pages:
                print(
                    f"Skipped TOC pages (only in first {self.toc_check_pages}): "
                    f"{self.skipped_pages}"
                )
            else:
            ...
            logger.exception("Error in PDF cleaning/merging pipeline: %s", e)
            raise PDFParseError("Failed during PDF cleaning/merging pipeline.") from e
