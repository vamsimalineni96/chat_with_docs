...
def get_customer_payments(
    ...
    Pass `product_filter` (case-insensitive substring) ONLY when the user is
    asking to act on a specific product (e.g. "refund the webcam"). When set:
      - If multiple payments match that product, this tool returns
        {requires_disambig: True, candidates: [...]} and the system pauses
        for the human to pick one. Do NOT try to pick one yourself.
      - If exactly one matches, it's returned as the only payment_intent.
    ...
    try:
        ...
        # Fetch a wider window so we can filter out failed/cancelled/refunded
        # payments. PaymentIntent.status stays "succeeded" after a refund —
        # the refund state lives on the latest Charge, so we expand it.
        ...
        def _is_refundable(p: Any) -> bool:
            if p.status != "succeeded":
                return False
            ch = getattr(p, "latest_charge", None)
            if ch is None:
                return True
            if getattr(ch, "refunded", False):
                return False
            amt_refunded = getattr(ch, "amount_refunded", 0) or 0
            return amt_refunded < (p.amount or 0)

        refundable = [p for p in payments.data if _is_refundable(p)]

        # Apply optional product filter (deterministic disambig).
        # Token-level matching with plural stripping so the agent can pass
        # natural phrases ("usb cables") and we still find "USB-C Cable 6ft".
        # Drops stopwords + trailing 's'; payment matches if ANY remaining
        # token appears in the description.
        if product_filter:
            stopwords = {"the", "a", "an", "all", "any", "for", "of", "my", "his", "her", "their", "this", "that"}
            raw_tokens = product_filter.strip().lower().split()
            tokens = [
            ...
            matched = [
                p for p in refundable
                if any(tok in (p.description or "").lower() for tok in tokens)
            ] if tokens else []
            candidates = [
                {
                    "id": p.id,
            ...
            if len(candidates) > 1:
                return {
                    ...
                    "requires_disambig": True,
                    "candidates": candidates,
                    "display": (
                        f"Found {len(candidates)} matching payments for "
                        f"'{product_filter}' — please pick which one to refund."
                    ),
                    "message": "Human disambiguation required before refund.",
                }
            # 0 or 1 matches — fall through to the normal shape with that subset.
            refundable = matched

        refundable = refundable[:limit]
        return {
            "customer": {
                "id": customer.id,
                "name": customer.name,
...
def create_refund(payment_intent_id: str, reason: str | None = None) -> dict[str, Any]:
    """Request a refund for a payment intent.

    This tool DOES NOT actually issue the refund — it returns a confirmation
    request that the system pauses on and surfaces to a human approver.
    Only after the human approves out-of-band will the refund be executed.

    Valid reasons (optional): duplicate, fraudulent, requested_by_customer.
    """
    if not stripe.api_key:
        return {"error": "Stripe API key not configured"}
    ...
    try:
        ...
        amount_str = f"${pi.amount / 100:.2f} {pi.currency.upper()}"
        return {
            ...
            "reason": reason,
            "display": f"Refund {amount_str} for payment {payment_intent_id}",
            "message": f"Confirmation required before issuing refund of {amount_str}.",
        }
    except stripe.StripeError as e:
        return {"error": str(e)}
