...
| Cost-per-successful-task metric | **Implemented.** `evals/cost/cost_report.py` aggregates Langfuse spans, joins against `evals/cost/pricing.py`, and emits a markdown report partitioned by task type (`cache-hit` vs `rag-full`) and pipeline stage. Sample output in `docs/reports/cost_sample.md`. Closed by PR #4. | [evals/cost/cost_report.py](../evals/cost/cost_report.py), [docs/reports/cost_sample.md](reports/cost_sample.md). |
...
| LLM-as-judge eval (decomposed dimensions, calibrated, sampled async) | **Implemented** (manual mode). Dataset ([evals/quality/qa_set.jsonl](../evals/quality/qa_set.jsonl), 18 HP4/HP7 pairs) + retrieval metrics ([evals/quality/metrics.py](../evals/quality/metrics.py)) + Llama 3.3 70B judge with YAML-driven rubric ([evals/quality/judge.py](../evals/quality/judge.py), [evals/quality/prompts/judge.yaml](../evals/quality/prompts/judge.yaml)) + markdown reporter ([evals/quality/reporter.py](../evals/quality/reporter.py)) + end-to-end orchestrator ([evals/quality/run_eval.py](../evals/quality/run_eval.py)). Sub-PR #7d adds the scheduled CI workflow + auto-commit of dated reports. Calibration against human labels is the only remaining deferred item. | [evals/quality/](../evals/quality/), [docs/eval-reports/eval_sample.md](eval-reports/eval_sample.md) |
| Heuristic checks (synchronous, deterministic) — refusal detection, citation validation, length bounds, format validation | **Implemented (refusal, citation, length).** Runs inline at the end of `answer_question`; tags the Langfuse trace with `heuristic_pass:true/false` and (on failure) `heuristic_failed:<check names>`. Surfaced in the `debug` response payload so the eval harness sees it too. Format validation is deferred — single-format-output system today. | [src/utils/services/heuristics.py](../src/utils/services/heuristics.py), [src/utils/rag_pipeline.py](../src/utils/rag_pipeline.py) — `_record_heuristics`. |
...
| P95/P99 rollup by stage and task type | **Implemented.** Nightly latency report aggregates Langfuse trace data into p50/p95/p99 by task type and pipeline stage. | [evals/latency/latency_report.py](../evals/latency/latency_report.py), [docs/reports/](reports/) |
| Time-to-first-token (TTFT) | **N/A.** Streaming is not yet wired on the `/chat` endpoint. | [app.py:61–170](../app.py). |
| Retry / fallback rate logging | **Implemented (retries).** Every NVIDIA call on the `/chat` read path is wrapped in `call_with_retry` — 3 attempts, exponential backoff capped at 15s, WARNING log per attempt. Same shared helper protects the ingest path. Fallback model still pending (PR #11). | [src/utils/services/retry.py](../src/utils/services/retry.py), call sites: [inference.py](../src/utils/services/inference.py), [embedder.py](../src/utils/services/embedder.py), [chunk_ranking.py](../src/utils/services/chunk_ranking.py). |
...
- **PR #6 — Eval harness v1.** `evals/quality/qa_set.jsonl` (15–25 HP4/HP7 Q&A pairs), `evals/quality/run_eval.py`, `evals/quality/metrics.py` (recall@k, MRR), `evals/quality/judge.py` (decomposed LLM-as-judge: groundedness, accuracy, completeness — using a *different* model family than the generator to avoid self-preference bias), `evals/quality/reporter.py` (markdown output), `docs/eval-reports/latest.md` committed back. *Maps to §3.3 row 2.*
...
- **PR #8 — Regression dataset.** Capture failures (heuristic-flagged + low judge score + user-flagged via a `/feedback` endpoint) into `evals/quality/regressions.jsonl`. Add a CI job that runs the regression set on any PR modifying `src/utils/services/`, `src/utils/rag_pipeline.py`, or `src/prompts/`. Pooja: "Regressions block deployment." *Maps to §3.3 row 4.*
...
- **PR #9 — Stage-level p50/p95/p99 latency report. Implemented.** `evals/latency/latency_report.py` pulls the past 7 days of Langfuse traces (v1 endpoints only — works on self-hosted + Cloud), computes percentiles by task type and pipeline stage, plus a slowest-traces top-10. Same shape as the cost report; stage classifiers are shared. Nightly cron at 06:30 UTC ([latency-report-nightly.yml](../.github/workflows/latency-report-nightly.yml)) commits dated reports to [`docs/reports/`](reports/). Alert thresholds (Pooja's retrieval p99 > 800ms, end-to-end p95 > 8s) are deferred to a follow-up PR once a few weeks of nightly data are accumulated. *Maps to §3.4 row 2.*
- **PR #10 — Retry on the /chat read path. Implemented (retries; circuit breakers deferred).** `src/utils/services/retry.py` — shared `call_with_retry(fn, *, op_name, max_attempts=3, max_sleep_s=15)` helper. Wired into `inference.NIMClient.chat_completion`, `embedder.EmbeddingHandler.get_embedding/get_document_embeddings`, and `chunk_ranking.NVidiaReranker.execute`. Every attempt logs at WARNING level with op_name + attempt count + error + sleep duration. Circuit breakers (`pybreaker` etc.) are deferred — the retry helper alone absorbs the most common upstream blip (a single 502/503 from NVIDIA), and we want a few weeks of nightly latency data before adding a state machine on top. *Maps to §3.4 row 4.*
